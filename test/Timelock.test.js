...
contract('Timelock', ([alice, bob, carol, dev, fee, minter]) => {
    beforeEach(async () => {
        this.h2o = await H2OToken.new({ from: alice });
        this.timelock = await Timelock.new(bob, '28800', { from: alice }); //8hours
    });

    it('should not allow non-owner to do operation', async () => {
        await this.h2o.transferOwnership(this.timelock.address, { from: alice });
        await expectRevert(
            this.h2o.transferOwnership(carol, { from: alice }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.h2o.transferOwnership(carol, { from: bob }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.timelock.queueTransaction(
                this.h2o.address, '0', 'transferOwnership(address)',
    ...
    it('should do the timelock thing', async () => {
        ...
        assert.equal((await this.h2o.owner()).valueOf(), carol);
    });
    ...
    it('should also work with MasterChef', async () => {
        ...
        this.chef = await MasterChef.new(this.h2o.address, dev, fee, '1000', '0', { from: alice });
        await this.h2o.transferOwnership(this.chef.address, { from: alice });
        ...
        await this.chef.add('100', this.lp1.address, 0, true, { from: minter })
        // await this.timelock.executeTransaction(
        //     this.chef.address, '0', 'add(uint256,address,bool)',
        //     encodeParameters(['uint256', 'address', 'bool'], ['100', this.lp2.address, false]), eta, { from: bob },
        // );
