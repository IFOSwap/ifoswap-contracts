...
contract('MasterChef', ([alice, bob, carol, dev, fee, minter]) => {
  beforeEach(async () => {
    ...
    this.chef = await MasterChef.new(this.h2o.address, dev, fee, '1000', '100', { from: minter });
  ...
  it('real case', async () => {
    ...
    assert.equal((await this.h2o.balanceOf(alice)).toString(), '0');
    await this.chef.deposit(1, '20', { from: alice });
    await this.chef.withdraw(1, '20', { from: alice });
    assert.equal((await this.h2o.balanceOf(alice)).toString(), '263');
    ...
    await this.chef.enterStaking('0', { from: alice });
    await this.chef.enterStaking('0', { from: alice });
    assert.equal((await this.h2o.balanceOf(alice)).toString(), '993');
  ...
  it('deposit/withdraw', async () => {
    ...
    assert.equal((await this.h2o.balanceOf(alice)).toString(), '999');
    assert.equal((await this.h2o.balanceOf(dev)).toString(), '140');
    ...
    assert.equal((await this.h2o.balanceOf(bob)).toString(), '125');
  ...
  it('staking/unstaking', async () => {
    ...
    await this.h2o.approve(this.chef.address, '250', { from: alice });
    await this.chef.enterStaking('240', { from: alice }); //3
    assert.equal((await this.h2o.balanceOf(alice)).toString(), '10');
    await this.chef.enterStaking('10', { from: alice }); //4
    assert.equal((await this.h2o.balanceOf(alice)).toString(), '249');
    await this.chef.leaveStaking(250);
    assert.equal((await this.h2o.balanceOf(alice)).toString(), '748');
  ...
  it('update multiplier', async () => {
    ...
    await this.h2o.approve(this.chef.address, '100', { from: alice });
    await this.h2o.approve(this.chef.address, '100', { from: bob });
    ...
    assert.equal((await this.h2o.balanceOf(alice)).toString(), '455');
    assert.equal((await this.h2o.balanceOf(bob)).toString(), '150');
