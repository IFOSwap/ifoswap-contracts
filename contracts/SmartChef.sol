...
contract SmartChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accH2OPerShare;   // Accumulated H2Os per share, times 1e22. See below.
    }
    ...
    // The block number when H2O mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 h2oReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accH2OPerShare = accH2OPerShare.add(h2oReward.mul(1e22).div(lpSupply));
        }
        return user.amount.mul(accH2OPerShare).div(1e22).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 h2oReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accH2OPerShare = pool.accH2OPerShare.add(h2oReward.mul(1e22).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        updatePool(0);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accH2OPerShare).div(1e22).sub(user.rewardDebt);
            if(pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accH2OPerShare).div(1e22);
